...
func NewGraph(uri string, skipVerify ...bool) *Graph {
	...
	g := &Graph{
		...
		indexBySubject:   make(map[string][]*Triple),
		indexByPredicate: make(map[string][]*Triple),
		indexByObject:    make(map[string][]*Triple),
	}
	return g
}
...
// All returns all triples that match a given pattern of S, P, O objects
func (g *Graph) All(s Term, p Term, o Term) []*Triple {
	...
	if s != nil {
		...
		// If no terms are provided, return all triples
		for triple := range g.triples {
